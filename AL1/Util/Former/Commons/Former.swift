...
public final class Former: NSObject {
    ...
    **/
    ...
    @objc private dynamic func keyboardWillAppear(notification: NSNotification) {
        ...
        // 2. 找到当前正在编辑的 Cell
        guard let firstResponder = findFirstResponder(tableView),
              let cell = findCellWithSubView(firstResponder),
              let indexPath = tableView.indexPath(for: cell) else { return }
        
        // 3. 计算键盘遮挡高度
        let keyboardFrame = keyboardFrameValue.cgRectValue
        // 转换坐标系，确保在不同屏幕和窗口模式下计算准确
        let convertedKeyboardFrame = tableView.superview?.convert(keyboardFrame, from: nil) ?? keyboardFrame
        
        let tableViewBottomY = tableView.frame.maxY
        let keyboardTopY = convertedKeyboardFrame.minY
        let bottomInset = max(0, tableViewBottomY - keyboardTopY)
    ...
    @objc private dynamic func keyboardWillDisappear(notification: NSNotification) {
        guard let userInfo = notification.userInfo,
              ...
              let originalInset = oldBottomContentInset else { return }
        
        let animationCurve = UIView.AnimationOptions(rawValue: UInt(curveValue << 16))
        
        UIView.animate(withDuration: duration, delay: 0, options: [animationCurve, .beginFromCurrentState], animations: {
            tableView.contentInset.bottom = originalInset
        }) { _ in
